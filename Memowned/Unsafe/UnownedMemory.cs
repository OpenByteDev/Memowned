...
namespace Memowned {
    ...
    public readonly struct UnownedMemory<T> : IMemoryOwner<T>, IReadOnlyMemoryOwner<T>, IEquatable<UnownedMemory<T>> {
        ...
        public override bool Equals(object? obj) => obj is UnownedMemory<T> memory && Equals(memory);
        ...
        public override string ToString() {
            ...
            return $"UnownedMemory<{typeof(T)}>[{Length.ToString()}]";
        }
        ...
        public static explicit operator OwnedSpan<T, UnownedMemory<T>>(UnownedMemory<T> memory) => OwnedSpan<T>.Owned(memory.Span, memory);
